...
const Extension = ({ context, runServerless, sendAlert }) => {  
  ...
  const handleClick = () => {
    runServerless({ name: "getData", parameters: { deviceNumber }  }).then((resp) =>{
  ...
  const searchReplacement = () => {
    ...
    runServerless({ name: "searchReplacement", parameters: { deviceNumber }  }).then((resp) =>{
  ...
  return (
    <>
      ...
      <Flex direction="row" align="end" gap="small">
        <Input name="text" label="Device number" onInput={(value) => setDeviceNumber(value)}  placeholder='Device number'/>
        <Button type="submit" onClick={handleClick} variant="primary"> Start a replacement procedure </Button>
      </Flex>
      <Text>
        {response && response.length !== 0 && response.map(resp =>(   
            <>
              <Text format={{ fontWeight: "bold" }}> Existing device : </Text>
              <Text>{resp.brand} {resp.model} {resp.releaseYear}</Text>
                <Image
      ...
      {response && response.length !== 0 && (
         <>
         <Button type="submit" onClick={searchReplacement} variant="primary"> Look for replacements</Button>
         
          <Text format={{ fontWeight: "bold" }}> Replacement possibilities : </Text>
